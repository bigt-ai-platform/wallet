...
## 8. Token creation requires domain signature

First-time issuances need genesis domain key signature. Otherwise saved as pending multisign. Need `pullBlockDoMultiSign` flow (Java `AbstractIntegrationTest`).
...
3. All 7 token tests pass (done) ✅
4. Remaining: implement `pullBlockDoMultiSign` in JS SDK for full token confirmation
